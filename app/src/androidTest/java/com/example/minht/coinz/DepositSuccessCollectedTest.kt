...
class DepositSuccessCollectedTest {
    ...
    fun setUp() {
        ...
        mAuth.signInWithEmailAndPassword("espresso@test.com","123456").addOnCompleteListener{task ->
            if (task.isSuccessful) {
                val uid = mAuth.uid
                Helper.prepareSuccessCollectedDeposit(uid!!)
            }
        }
    }
    // Test that depositing only collected coins without breaking limit is successful
    @Test
